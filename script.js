...
function handleGoogleResponse(response) {
  ...
  if (payload) {
    ...
    saveUserSession(payload.name, payload.email, payload.picture);
...
function fetchGoogleUserInfo(accessToken) {
  fetch('https://www.googleapis.com/oauth2/v2/userinfo', {
    ...
    .then((data) => {
      // Store user session and redirect to homepage
      saveUserSession(data.name, data.email, data.picture);
      showToast('Logged in successfully! Redirecting...', 'success');
      setTimeout(() => { window.location.href = 'homepage.html'; }, 800);
    })
    .catch(() => {
...
function handleEmailLogin() {
  ...
  // Extract a display name from the email (before the @)
  const displayName = email.split('@')[0].replace(/[._-]/g, ' ').replace(/\b\w/g, c => c.toUpperCase());

  // Store user session
  saveUserSession(displayName, email, '');

  // Show success and redirect to homepage
  const loginBtn = document.getElementById('loginBtn');
...
function saveUserSession(name, email, picture) {
  const session = {
    name: name || 'User',
    email: email || '',
    picture: picture || '',
  ...
  localStorage.setItem('gooruSession', JSON.stringify(session));
}

function getUserSession() {
  try {
    const data = localStorage.getItem('gooruSession');
...
function clearUserSession() {
  localStorage.removeItem('gooruSession');
}
